...
| --- | --- | ---: | --- |
| `feature/sim-sdk/neutral-conformance-packs` | `crate/sim-conformance-packs` | 1 | Expose all 21 checker entrypoints with pure evidence-driven scenarios, canonical results, and typed unavailable outcomes for scopes whose funded phase has not shipped, including canonical identity, native journal, durable operation reconciliation, and the real local checker boundary. |
