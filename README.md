...
Enable the non-default `study` domain bundle to re-export the canonical model
observatory as `sim::model_test` alongside the granular study and provider
facades. It remains absent from the minimal facade and adds no
provider authority, credentials, runner, scheduler, statistics, or private
model inventory.

The opt-in `estate` domain bundle exports the portable estate core, authoring,
organ, serving, Surface, and deterministic model-provider facades; private
bindings and concrete controller capsules remain outside default features.
